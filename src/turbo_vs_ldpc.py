...
This version is written to work with:
- the split turbo project,
- the optimized LDPC project,
- slightly different naming choices across config.py and simulation.py.
...
# Import helpers
# ---------------------------------------------------------------------
# Both repositories use generic module names such as config.py, encoder.py,
# decoder.py and simulation.py. To avoid cross-contamination between imports,
# this script loads one project at a time and clears those names afterwards.
PROJECT_MODULE_NAMES = ["config", "encoder", "decoder", "simulation", "main"]


def clear_project_modules():
    """Remove local repository modules so the next project imports cleanly."""
...
def get_first_existing_attribute(module, candidate_names, module_label):
    """Return the first matching attribute from a list of possible names."""
...
# ---------------------------------------------------------------------
# Turbo project loader
# ---------------------------------------------------------------------
def run_turbo_project(turbo_folder: Path):
    """Run the turbo project and collect data needed for comparison plots."""
    ...
    with TemporaryProjectPath(turbo_folder):
        ...
        turbo_data = {
            ...
            "turbo_ber_by_iteration": normalize_iteration_results(
...
def run_ldpc_project(ldpc_folder: Path):
    """Run the LDPC project and collect data needed for comparison plots."""
    ...
    with TemporaryProjectPath(ldpc_folder):
        ...
        if not isinstance(simulation_output, tuple) or len(simulation_output) < 2:
            raise ValueError(
                "LDPC simulation must return at least (ber_results, llr_snapshot)."
            )
        ...
        ldpc_data = {
            ...
            "ldpc_ber_by_iteration": normalize_iteration_results(
...
def uncoded_bpsk_ber(ebn0_db_values):
    """Theoretical BER for uncoded BPSK in AWGN."""
...
# ---------------------------------------------------------------------
# Plot 1: Equal-iteration overlay
# ---------------------------------------------------------------------
def plot_equal_iteration_overlay(turbo_data, ldpc_data):
    """Plot turbo and LDPC BER curves for the same iteration counts."""
    floor_value = 1e-8
    plt.figure(figsize=(8.2, 5.2))

    for iteration_count in ITERATIONS_TO_PLOT:
        if iteration_count in turbo_data["turbo_ber_by_iteration"]:
            plt.semilogy(
                turbo_data["turbo_ebn0_db"],
                np.clip(
                    turbo_data["turbo_ber_by_iteration"][iteration_count],
                    floor_value,
                    None,
                ),
                marker="o",
        ...
        if iteration_count in ldpc_data["ldpc_ber_by_iteration"]:
            plt.semilogy(
                ...
                np.clip(
                    ldpc_data["ldpc_ber_by_iteration"][iteration_count],
                    floor_value,
                    None,
                ),
                marker="x",
    ...
    plt.legend(ncol=2)
    finish_figure(f"{FIGURE_PREFIX}_iterations.png")


# ---------------------------------------------------------------------
# Plot 2: Best curves with baselines
# ---------------------------------------------------------------------
def plot_best_curves_with_baselines(turbo_data, ldpc_data):
    """Plot the best turbo and LDPC curves with uncoded and convolutional references."""
    floor_value = 1e-8
    best_turbo_iteration = max(turbo_data["turbo_iteration_list"])
    best_ldpc_iteration = max(ldpc_data["ldpc_iteration_list"])

    uncoded_curve = uncoded_bpsk_ber(turbo_data["convolutional_ebn0_db"])
    ...
    plt.semilogy(
        turbo_data["turbo_ebn0_db"],
        np.clip(
            turbo_data["turbo_ber_by_iteration"][best_turbo_iteration],
            floor_value,
            None,
        ),
        "ko-",
    ...
    plt.semilogy(
        ldpc_data["ldpc_ebn0_db"],
        np.clip(
            ldpc_data["ldpc_ber_by_iteration"][best_ldpc_iteration],
            floor_value,
            None,
        ),
        color="tab:green",
    ...
    plt.grid(True, which="both", alpha=0.35)
    plt.legend()
    finish_figure(f"{FIGURE_PREFIX}_best_curves.png")


# ---------------------------------------------------------------------
# Plot 3: Decoder confidence growth
# ---------------------------------------------------------------------
def plot_decoder_confidence_growth(turbo_data, ldpc_data):
    """Compare the posterior LLR growth of the turbo and LDPC decoders."""
    plt.figure(figsize=(10.5, 4.6))

    plt.subplot(1, 2, 1)
    for iteration_count in turbo_data["turbo_iteration_list"]:
    ...
    plt.subplot(1, 2, 2)
    for iteration_count in ldpc_data["ldpc_iteration_list"]:
    ...
    finish_figure(f"{FIGURE_PREFIX}_llr_growth.png")


# ---------------------------------------------------------------------
# Printed summary
# ---------------------------------------------------------------------
def print_quick_summary(turbo_data, ldpc_data):
    """Print a compact numerical summary at common Eb/N0 points."""
    best_turbo_iteration = max(turbo_data["turbo_iteration_list"])
    best_ldpc_iteration = max(ldpc_data["ldpc_iteration_list"])
    ...
    if turbo_data["information_length"] is not None:
        print(f"Turbo information length: {turbo_data['information_length']}")
    if ldpc_data["information_length"] is not None:
        print(f"LDPC information length: {ldpc_data['information_length']}")

    if ldpc_data["code_rate"] is not None:
        print(f"LDPC code rate: {ldpc_data['code_rate']}")
    ...
    if common_ebn0_values.size == 0:
        print("No common Eb/N0 points were found between the two projects.")
        return
    ...
    for ebn0_value in common_ebn0_values:
        ...
        lower_label = "Turbo" if turbo_ber < ldpc_ber else "LDPC"

        print(f"{ebn0_value:10.2f} {turbo_ber:14.4e} {ldpc_ber:14.4e} {lower_label:>12}")
...
def main():
    ...
    turbo_data = run_turbo_project(TURBO_FOLDER)

    print("\nRunning LDPC simulation...")
    ldpc_data = run_ldpc_project(LDPC_FOLDER)

    plot_equal_iteration_overlay(turbo_data, ldpc_data)
