...
This script temporarily patches each project's config.py to the requested code
rate and decoder iteration list, runs the simulations, and then restores the
...
class TemporaryConfigPatch:
    ...
    def __enter__(self):
        ...
        patched_text = patch_config_text(self.original_text, self.rate_label, self.iteration_list)
...
def normalize_iteration_results(raw_results, iteration_list):
    if isinstance(raw_results, dict):
        return {int(iteration_count): np.array(raw_results[iteration_count], dtype=float) for iteration_count in iteration_list}
    if isinstance(raw_results, (list, tuple)):
        ...
        return {int(iteration_list[index]): np.array(raw_results[index], dtype=float) for index in range(len(iteration_list))}
    ...
    with TemporaryConfigPatch(TURBO_FOLDER, rate_label):
        with TemporaryProjectPath(TURBO_FOLDER):
            ...
            return {
                ...
                "ber_by_iteration": normalize_iteration_results(turbo_results, turbo_config.DECODER_ITERATION_LIST),
    ...
    with TemporaryConfigPatch(LDPC_FOLDER, rate_label):
        with TemporaryProjectPath(LDPC_FOLDER):
            ...
            return {
                ...
                "ber_by_iteration": normalize_iteration_results(ldpc_results, ldpc_config.DECODER_ITERATION_LIST),
