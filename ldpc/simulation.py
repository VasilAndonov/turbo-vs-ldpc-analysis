...
from ldpc.encoder import build_qc_ldpc_matrices, build_edge_structure, encode_qc_ldpc
from ldpc.decoder import decode_ldpc_normalized_minsum

def simulate_ldpc(rate_label=SELECTED_CODE_RATE_LABEL):
    rng = np.random.default_rng(RANDOM_SEED)
    h_matrix, a_proto, b_proto, z_size, codeword_bits, _ = build_qc_ldpc_matrices(rate_label)
    edge_variable, check_edge_start, variable_edges, variable_edge_start = build_edge_structure(h_matrix)
    ...
    for ebn0_db in LDPC_EBN0_DB:
        ...
        while frames < MIN_FRAMES or (errors[max(ITERATIONS)] < TARGET_ERRORS and frames < MAX_FRAMES):
            ...
            codeword = encode_qc_ldpc(bits, a_proto, b_proto, z_size)
            tx = 1.0 - 2.0 * codeword
            rx = tx + sigma * rng.standard_normal(codeword_bits)

            history = decode_ldpc_normalized_minsum(
                rx,
                sigma2,
                max(ITERATIONS),
                h_matrix,
